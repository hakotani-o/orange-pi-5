...
mkdir $1/aaa
chroot $1 apt-get -y install kubuntu-desktop yakuake sddm linux-firmware snapd cloud-initramfs-growroot oem-config-gtk ubiquity-frontend-gtk aptdaemon grub-efi-arm64 initramfs-tools yaru-theme-unity yaru-theme-icon yaru-theme-gtk ubiquity-slideshow-kubuntu plymouth-theme-kubuntu-logo 
...
# mesa
mkdir $1/bbb
chroot $1 /bin/bash -c "cd bbb && git clone --depth 1 https://gitlab.freedesktop.org/mesa/libdrm.git && cd libdrm/ && mkdir build && cd build/ && meson && ninja install"
